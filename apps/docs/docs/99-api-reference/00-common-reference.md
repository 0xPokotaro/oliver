...
Oliver APIは、以下の3つのカテゴリに分類されます：

| カテゴリ名 | パス | 説明 |
|----------|------|------|
| **ユーザーカテゴリ** | `/v1/user` | ユーザー情報の取得と音声コマンド実行 |
| **AIカテゴリ** | `/v1/agent` | AIエージェント向けの機能（将来拡張予定） |
| **決済カテゴリ** | `/v1/commerce` | 商品情報の取得、x402決済プロトコルを使用した商品購入、注文確認 |
