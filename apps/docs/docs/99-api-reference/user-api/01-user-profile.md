...
**購入履歴を除外したリクエスト:**
```bash
curl -X GET "http://localhost:8080/api/v1/user/profile?includeHistory=false" \
  -H "Authorization: Bearer <JWT_TOKEN>" \
  -H "Content-Type: application/json"
```
...
**購入履歴の件数を指定したリクエスト:**
```bash
curl -X GET "http://localhost:8080/api/v1/user/profile?historyLimit=5" \
  -H "Authorization: Bearer <JWT_TOKEN>" \
  -H "Content-Type: application/json"
```
...
**購入履歴を除外する例:**
```javascript
const response = await fetch('/api/v1/user/profile?includeHistory=false', {
  method: 'GET',
  headers: {
    'Authorization': `Bearer ${jwtToken}`,
    'Content-Type': 'application/json'
...
**購入履歴の件数を指定する例:**
```javascript
const response = await fetch('/api/v1/user/profile?historyLimit=5', {
  method: 'GET',
  headers: {
    'Authorization': `Bearer ${jwtToken}`,
    'Content-Type': 'application/json'
